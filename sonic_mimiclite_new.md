...
- **实测重建误差**（关节角度，逐帧对比编码前后的差异，見下方"重建误差基准对照"）：
  在第 12000 轮測得 mean=13.99°、p95=34.36°（`walk_forward_loop_003__A022` 一条动作）。
  用同一方法测了三个参照点：G1 官方发布 checkpoint（mean=6.44°，成熟/发布级别）、
  我们自己的 BUMI3 训练在 3 万轮时（mean=18.10°，训练目标 10 万轮的中间点）、
  BUMI3 练满 10 万轮时（mean=12.68°，真正训练终点）。**BUMI2 只训 17618 轮就已经
  追平 BUMI3 练满 10 万轮的水平**，真正的差距是相对 G1 官方版本，且这个差距大概率
  不是"多训练就能补上"的（BUMI3 练满全程也没追上 G1）。
...
10. 跑通 sim2sim，验证 MimicLite-BUMI2 策略能不能按照我们桥接出来的参考动作实际
    追踪；如果追不上，回头判断是 §6 重建质量不够（当前实测约 13~14° 平均关节角度
    误差，详见 §3），还是桥接格式转换本身有问题。
