...
class PPOConfig:
    ...
    # Exact resumes normally restore the absolute schedule boundaries saved in
    # the checkpoint.  When extending a run to a new total_iters, rebase those
    # boundaries from the configured fractions instead.
    resume_rebase_schedules: bool = False
...
class PPOPolicy(PPOBase):
    ...
    def state_dict(self):
        ...
        state_dict["training_state"] = {
            ...
            "entropy_decay_start": self.entropy_decay_start,
            "entropy_decay_end": self.entropy_decay_end,
        }
        return state_dict
    ...
    def load_state_dict(self, state_dict, strict=True):
        ...
        training_state = state_dict.get("training_state")
        if training_state is None:
            warnings.warn(
                "Legacy PPO checkpoint has no optimizer or schedule state; "
                "model parameters were loaded, but this is a warm start rather "
                "than an exact training resume."
            )
        else:
            try:
                self.opt_policy.load_state_dict(training_state["opt_policy"])
                self.opt_critic.load_state_dict(training_state["opt_critic"])
                self.lr_policy = float(training_state["lr_policy"])
                for param_group in self.opt_policy.param_groups:
                    param_group["lr"] = self.lr_policy
                self.num_updates = int(training_state["num_updates"])
                if self.cfg.resume_rebase_schedules:
                    # __init__ already derived the absolute boundaries from the
                    # configured fractions and the new total_iters.  Keep them,
                    # and let train_policy recompute entropy/KL at current_iter.
                    print(
                        "Successfully restored PPO optimizer, learning-rate, "
                        "and update-count; rebased entropy/KL schedules to "
                        "the new total_iters."
                    )
                else:
                    self.entropy_coef = float(training_state["entropy_coef"])
                    self.desired_kl = training_state["desired_kl"]
                    self.entropy_decay_start = float(
                        training_state["entropy_decay_start"]
                    )
                    self.entropy_decay_end = float(
                        training_state["entropy_decay_end"]
                    )
                    print(
                        "Successfully restored PPO optimizer, learning-rate, "
                        "update-count, and schedule state."
                    )
            except Exception as exc:
                if strict:
                    raise RuntimeError(
                        f"Strict PPO training-state load failed: {exc}"
                    ) from exc
                warnings.warn(f"Failed to restore PPO training state: {exc}")
