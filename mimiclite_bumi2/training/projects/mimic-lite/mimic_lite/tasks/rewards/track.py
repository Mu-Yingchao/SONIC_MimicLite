...
from mimic_lite.tasks.transforms import projected_yaw_quat, quat_apply_inverse
...
class waist_com_feet_support_x_error_exp(TrackReward, namespace="mimic_lite"):
    """Keep the waist COM projection near the midpoint of both feet in local X."""
    ...
    def _initialize_impl(
        ...
        left_foot_body_name: str = "l_ankle_roll_link",
        right_foot_body_name: str = "r_ankle_roll_link",
        **kwargs,
    ):
        if sigma <= 0.0:
            raise ValueError(f"sigma must be positive, got {sigma}")

        tracking_body_names = list(self.command_manager.tracking_body_names)
        required_body_names = (
            waist_body_name,
            left_foot_body_name,
            right_foot_body_name,
        )
        missing = [name for name in required_body_names if name not in tracking_body_names]
        if missing:
            raise ValueError(
                f"Bodies {missing} are not in tracking_body_names: {tracking_body_names}"
            )

        self.sigma = sigma
        self.target_x = target_x
        self.waist_idx_tracking = tracking_body_names.index(waist_body_name)
        self.left_foot_idx_tracking = tracking_body_names.index(left_foot_body_name)
        self.right_foot_idx_tracking = tracking_body_names.index(right_foot_body_name)
        self.waist_idx_asset = self.command_manager.tracking_body_indices_asset[
            self.waist_idx_tracking
        ]

    def _compute(self):
        waist_com_w = self.command_manager.asset.data.body_com_pos_w[
            :, self.waist_idx_asset
        ]
        feet_center_w = 0.5 * (
            self.command_manager.robot_body_link_pos_w[:, self.left_foot_idx_tracking]
            + self.command_manager.robot_body_link_pos_w[:, self.right_foot_idx_tracking]
        )
        waist_yaw_quat_w = projected_yaw_quat(
            self.command_manager.robot_body_link_quat_w[:, self.waist_idx_tracking]
        )
        com_from_feet_b = quat_apply_inverse(
            waist_yaw_quat_w, waist_com_w - feet_center_w
        )
        error_sq = torch.square(com_from_feet_b[:, 0] - self.target_x)
        return torch.exp(-error_sq / self.sigma**2).unsqueeze(1)
