...
MOTION_ROOT = Path("/data2/zcx/datasets/any4hdmi-bumi-v2/motions/score1")
CHECKPOINT = Path(
    "/data2/zcx/outputs/2026-09-04/20-59-38-train-sequential/"
    "stages/03-finetune/checkpoint_4000.pt"
)
OUT = ROOT / "outputs/selected7_score1_mujoco_ppo_roa4000_corrected_onepass"
RUNS = OUT / "runs"
VIDEOS = OUT / "videos"
STEP_DT = 0.02
RELATIVE_MOTIONS = [
...
def main() -> None:
    ...
    for index, relative in enumerate(RELATIVE_MOTIONS, 1):
        motion = MOTION_ROOT.parent / relative
        ...
        cmd = [
            ...
            f"task.max_episode_length={steps + 100}",
            "task.termination.root_pos_error.enabled=false",
            "~task.termination.body_pos_error",
            "~task.termination.root_ori_error",
            "~task.termination.body_ori_error",
        ...
        with (run_dir / "console.log").open("w", encoding="utf-8") as log:
            try:
                ...
                code = 124
                log.write("\nBatch timeout after 900 seconds.\n")
