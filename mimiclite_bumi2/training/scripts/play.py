...
def main(cfg: PlayConfig):
    ...
    def _load_checkpoint(path: str) -> None:
        ...
        rollout_holder["policy"] = policy.get_rollout_policy("eval").to(env.device)
    ...
    if viewer is not None and cfg.get("checkpoint_path"):
        ...
        resolved_ckpt = parse_checkpoint_path(cfg.checkpoint_path)
        if resolved_ckpt is not None:
            viewer.attach_checkpoints(resolved_ckpt, on_load=_load_checkpoint)
