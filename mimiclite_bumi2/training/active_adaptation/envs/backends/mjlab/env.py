...
class MjlabBackendEnv(_EnvBase):
    ...
    def setup_scene(self):
        ...
        contact_sensor_ids = [
            ...
            if mj_model.sensor_type[sensor_id]
            == mujoco.mjtSensor.mjSENS_CONTACT
        ]
        contact_sensor_map = {
            contact_sensor_id: mujoco.mj_id2name(
                mj_model, mujoco.mjtObj.mjOBJ_SENSOR, sensor_id
            )
            for contact_sensor_id, sensor_id in enumerate(contact_sensor_ids)
        }
        logging.info(
            "MuJoCo Warp contactsensorid map: %s",
            contact_sensor_map,
        )
        sim = Simulation(
            ...
            cfg=SimulationCfg(
                ...
                contact_sensor_maxmatch=self.cfg.sim.get(
