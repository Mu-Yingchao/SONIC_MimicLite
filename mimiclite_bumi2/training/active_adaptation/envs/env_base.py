...
class _EnvBase(EnvBase, RegistryMixin):
    def __init__(self, cfg: EnvConfig, device: str, headless: bool = True):
        ...
        self._nonfinite_motion_ids_logged: set[int] = set()
        self._nonfinite_rows_previous = torch.zeros(
            self.num_envs,
            dtype=torch.bool,
            device=self.device,
        )
    ...
    def get_reward_term_names(self) -> list[str]:
        """Flat reward term names as ``group/term`` for viewer overlays."""
        names: list[str] = []
        for group_name, group in self.reward_groups.items():
            for term_name in group.funcs:
                names.append(f"{group_name}/{term_name}")
        return names

    def get_reward_terms_for_env(self, env_idx: int) -> list[tuple[str, np.ndarray]]:
        """Latest per-term reward values for one env (for Viser overlays)."""
        terms: list[tuple[str, np.ndarray]] = []
        idx = int(env_idx)
        for group_name, group in self.reward_groups.items():
            for term_name, func in group.funcs.items():
                last = getattr(func, "_last_rew", None)
                if last is None or last.numel() == 0:
                    value = np.zeros(1, dtype=np.float32)
                else:
                    value = last[idx].detach().float().cpu().numpy().reshape(-1)
                terms.append((f"{group_name}/{term_name}", value))
        return terms

    def _log_nonfinite_motion_rows(self, invalid: torch.Tensor) -> None:
        """Log the first occurrence of motions that produce non-finite rows."""
        newly_invalid = invalid & ~self._nonfinite_rows_previous
        self._nonfinite_rows_previous.copy_(invalid)
        if (
            not newly_invalid.any()
            or len(self._nonfinite_motion_ids_logged) >= 64
        ):
            return
        ...
        motion_paths = getattr(dataset, "motion_paths", None)
        if motion_ids is None or motion_t is None or motion_paths is None:
            return

        env_ids = newly_invalid.nonzero(as_tuple=False).squeeze(-1)
        env_ids_cpu = env_ids.detach().cpu().tolist()
        motion_ids_cpu = motion_ids[env_ids].detach().cpu().tolist()
        motion_t_cpu = motion_t[env_ids].detach().cpu().tolist()
        episode_ids_cpu = self.episode_id[env_ids].detach().cpu().tolist()

        for env_id, episode_id, motion_id, time_step in zip(
            env_ids_cpu,
            episode_ids_cpu,
            motion_ids_cpu,
            motion_t_cpu,
            ...
            if motion_id in self._nonfinite_motion_ids_logged:
                continue
            if len(self._nonfinite_motion_ids_logged) >= 64:
                break
            self._nonfinite_motion_ids_logged.add(motion_id)
            motion_path = (
                str(motion_paths[motion_id])
                if 0 <= motion_id < len(motion_paths)
                else "<out-of-range>"
            )
            logging.warning(
                "Non-finite env row: iter=%d env=%d episode_id=%d "
                "motion_id=%d motion_t=%d motion_path=%s",
                self.current_iter,
                env_id,
                episode_id,
                motion_id,
    ...
    def _step(self, tensordict: TensorDictBase) -> TensorDictBase:
        ...
        self._log_nonfinite_motion_rows(invalid)
